...
task :test_race => %w[fmt lint vet] do
  system("go build -race ./pkg/logagent") || exit(1)
  system("go test ./pkg/...") || exit(1)
end
...
task :build => %w[fmt lint vet] do
  system("go build -o build/logagent ./pkg/logagent") || exit(1)
end

desc "Build the agent on linux amd64"
task :build_linux_amd64 do
  puts("building for linux amd64")
  system("env GOOS=linux GOARCH=amd64 go build -o build/linux-amd64 ./pkg/logagent") || exit(1)
end
...
task :build_windows_amd64 do
  ...
  puts("Not supported")
  # system("env GOOS=windows GOARCH=amd64 go build -o build/windows-amd64 ./pkg/logagent") || exit(1)
end
...
task :install do
    system("go install ./pkg/logagent") || exit(1)
end
